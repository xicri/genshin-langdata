import type { SourceWord } from "../../libs/types.ts";

export default [
  //
  // Mondstadt
  //
  {
    en: "Floral Whisper",
    ja: "花言葉",
    zhCN: "花语",
    zhTW: "花語",
    tags: [ "mondstadt", "facility" ],
  },
  {
    en: "Mondstadt General Goods",
    ja: "モンドショップ",
    zhTW: "蒙德百货",
    zhCN: "蒙德百貨",
    tags: [ "mondstadt", "facility" ],
  },
  {
    en: "Angel's Share",
    ja: "エンジェルズシェア",
    zhCN: "天使的馈赠",
    zhTW: "天使的饋贈",
    tags: [ "mondstadt", "facility" ],
  },
  {
    en: "Cat's Tail",
    ja: "キャッツテール",
    zhCN: "猫尾酒馆",
    zhTW: "貓尾酒館",
    tags: [ "mondstadt", "facility" ],
  },
  {
    en: "Good Hunter",
    ja: "鹿狩り",
    zhCN: "猎鹿人",
    zhTW: "獵鹿人",
    tags: [ "mondstadt", "facility" ],
  },
  {
    en: "With Wind Comes Glory",
    ja: "栄光の風",
    zhCN: "荣光之风",
    zhTW: "榮光之風",
    tags: [ "mondstadt", "facility" ],
    notes: "風の印をアイテムと交換できる店",
  },
  {
    en: "Favonius Cathedral",
    ja: "西風大聖堂",
    zhCN: "西风大教堂",
    zhTW: "西風大教堂",
    tags: [ "mondstadt", "facility" ],
  },

  //
  // Liyue
  //
  {
    en: "Yujing Terrace",
    ja: "玉京台",
    zhCN: "玉京台",
    zhTW: "玉京臺",
    pronunciationJa: "ぎょくけいだい",
    tags: [ "liyue", "facility" ],
  },
  {
    en: "Yuehai Pavilion",
    ja: "月海亭",
    zhCN: "月海亭",
    zhTW: "月海亭",
    pronunciationJa: "げつかいてい",
    tags: [ "liyue", "facility" ],
  },
  {
    en: "Wangsheng Funeral Parlor",
    ja: "往生堂",
    zhCN: "往生堂",
    zhTW: "往生堂",
    pronunciationJa: "おうじょうどう",
    tags: [ "liyue", "facility" ],
  },
  {
    en: "Wanmin Restaurant",
    ja: "万民堂",
    zhCN: "万民堂",
    zhTW: "萬民堂",
    pronunciationJa: "ばんみんどう",
    tags: [ "liyue", "facility" ],
    notes: "璃月港にある食堂",
  },
  {
    en: "Bubu Pharmacy",
    ja: "不卜廬",
    zhCN: "不卜庐",
    zhTW: "不卜廬",
    pronunciationJa: "ふぼくろ",
    tags: [ "liyue", "facility" ],
    notes: "璃月港にある薬屋",
    variants: {
      ja: [ "不卜盧" ],
    },
  },
  {
    en: "Wanwen Bookhouse",
    ja: "万文集舎",
    zhCN: "万文集舍",
    zhTW: "萬文集舍",
    pronunciationJa: "まんぶんしゅうしゃ",
    tags: [ "liyue", "facility" ],
    notes: "璃月港にある書店",
  },
  {
    en: "Yanshang Teahouse",
    ja: "岩上茶室",
    zhCN: "岩上茶室",
    zhTW: "岩上茶室",
    pronunciationJa: "がんじょうちゃしつ",
    tags: [ "liyue", "facility" ],
    notes: "璃月港にある茶屋",
  },
  {
    en: "Liuli Pavilion",
    ja: "琉璃亭",
    zhCN: "琉璃亭",
    zhTW: "琉璃亭",
    pronunciationJa: "るりてい",
    tags: [ "liyue", "facility" ],
    notes: "璃月港にある店",
    variants: {
      ja: [ "瑠璃亭" ],
    },
  },
  {
    en: "Xinyue Kiosk",
    ja: "新月軒",
    zhCN: "新月轩",
    zhTW: "新月軒",
    pronunciationJa: "しんげつけん",
    tags: [ "liyue", "facility" ],
    notes: "璃月港にある店",
  },
  {
    en: "Third-Round Knockout",
    ja: "三杯酔",
    zhCN: "三碗不过港",
    zhTW: "三碗不過港",
    pronunciationJa: "さんばいよい",
    tags: [ "liyue", "facility" ],
    notes: "璃月港にある酒場",
  },
  {
    en: "Hanfeng's Ironmongers",
    ja: "寒鋒鉄器",
    zhCN: "寒锋铁器",
    zhTW: "寒鋒鐵器",
    pronunciationJa: "かんほうてっき",
    tags: [ "liyue", "facility" ],
  },
  {
    en: "Heyu Tea House",
    ja: "和裕茶館",
    zhCN: "和裕茶馆",
    zhTW: "和裕茶館",
    tags: [ "liyue", "facility" ],
    notes: "璃月港にある茶館",
  },
  {
    en: "Scent of Spring",
    ja: "春香窯",
    zhCN: "春香窑",
    zhTW: "春香窯",
    tags: [ "liyue", "facility" ],
    notes: "璃月港にある鶯の店",
  },
  {
    en: "The Jade Mystery",
    ja: "解翠行",
    zhCN: "解翠行",
    zhTW: "解翠行",
    tags: [ "liyue", "facility" ],
    notes: "璃月港にある石商の鉱石店",
  },
  {
    en: "Mingxing Jewelry",
    ja: "明星斎",
    zhCN: "明星斋",
    zhTW: "明星齋",
    pronunciationJa: "みょうじょうさい",
    tags: [ "liyue", "facility" ],
    notes: "璃月港にある岩の印をアイテムと交換できる店",
  },
  {
    en: "Baiju Guesthouse",
    ja: "白駒逆旅",
    zhCN: "白驹逆旅",
    zhTW: "白駒逆旅",
    tags: [ "liyue", "facility" ],
    notes: "璃月港にある旅館",
  },
  {
    en: "Xigu Antiques",
    ja: "希古居",
    zhCN: "希古居",
    zhTW: "希古居",
    tags: [ "liyue", "facility" ],
    notes: "璃月港にある店",
  },
  {
    en: "Wanyou Boutique",
    ja: "万有商舗",
    zhCN: "万有铺子",
    zhTW: "萬有鋪子",
    tags: [ "liyue", "facility" ],
    notes: "璃月港にある店",
  },

  //
  // Inazuma
  //
  {
    en: "Naganohara Fireworks",
    ja: "長野原花火屋",
    zhCN: "长野原烟花店",
    zhTW: "長野原煙花店",
    pronunciationJa: "ながのはらはなびや",
    tags: [ "inazuma", "facility" ],
  },
  {
    en: "Komore Teahouse",
    ja: "木漏茶屋",
    zhCN: "木漏茶室",
    zhTW: "木漏茶室",
    pronunciationJa: "こもれちゃや",
    tags: [ "inazuma", "facility" ],
  },
  {
    en: "Yae Publishing House",
    ja: "八重堂",
    zhCN: "八重堂",
    zhTW: "八重堂",
    pronunciationJa: "やえどう",
    notes: "稲妻城にある本屋および出版社",
    tags: [ "inazuma", "organization", "facility" ],
  },
  {
    en: "Bantan Sango Detective Agency",
    ja: "万端珊瑚探偵所",
    zhCN: "万端珊瑚侦探社",
    zhTW: "萬端珊瑚偵探社",
    pronunciationJa: "ばんたんさんごたんていしょ",
    tags: [ "inazuma", "facility" ],
  },
  {
    en: "Amenoma Smithy",
    ja: "天目鍛冶屋",
    zhCN: "天目锻冶屋",
    zhTW: "天目鍛冶屋",
    pronunciationJa: "あめのまかじや",
    tags: [ "inazuma", "facility" ],
  },
  {
    en: "Netsuke no Gen Crafts",
    ja: "根付の源",
    zhCN: "根付之源",
    zhTW: "根付之源",
    pronunciationJa: "ねつけのげん",
    tags: [ "inazuma", "facility" ],
    notes: "雷の印をアイテムと交換できる店",
  },
  {
    en: "Uyuu Restaurant",
    ja: "烏有亭",
    zhCN: "乌有亭",
    zhTW: "烏有亭",
    pronunciationJa: "うゆうてい",
    tags: [ "inazuma", "facility" ],
  },
  {
    en: "Shimura's",
    ja: "志村屋",
    zhCN: "志村屋",
    zhTW: "志村屋",
    pronunciationJa: "しむらや",
    tags: [ "inazuma", "facility" ],
  },
  {
    en: "Outlander Affairs Agency",
    ja: "遠国監察",
    zhCN: "远国监司",
    zhTW: "遠國監司",
    tags: [ "inazuma", "facility", "organization" ],
  },
  {
    en: "Syavushi Bakshesh Shoppe",
    ja: "豊穣の願い",
    zhCN: "葱茏之愿",
    zhTW: "蔥蘢之願",
    pronunciationJa: "ほうじょうのねがい",
    tags: [ "inazuma", "facility" ],
  },
  {
    en: "Sailing Breeze",
    ja: "セーリングブリーズ",
    zhCN: "远航之风",
    zhTW: "遠航之風",
    tags: [ "inazuma", "facility" ],
  },
  {
    en: "Aisa Bathhouse",
    ja: "秋沙銭湯",
    zhCN: "秋沙钱汤",
    zhTW: "秋沙錢湯",
    pronunciationJa: "あいさせんとう",
    tags: [ "inazuma", "facility" ],
  },
  {
    en: "Chuyu Trading Company",
    ja: "楚瑜商行",
    zhCN: "楚瑜商行",
    zhTW: "楚瑜商行",
    tags: [ "inazuma", "facility" ],
    notes: "離島にある店。【注意】日本語表示ではフォント制約により「瑜」の右下を「刂」表示できません",
  },
  {
    en: "Eipekkusu",
    ja: "天井屋",
    zhCN: "天顶屋",
    zhTW: "天頂屋",
    pronunciationJa: "てんじょうや",
    tags: [ "inazuma", "facility" ],
    notes: "離島にある店",
  },
  {
    en: "Kuminami Restaurant",
    ja: "木南料亭",
    zhCN: "木南料亭",
    zhTW: "木南料亭",
    pronunciationJa: "こなんりょうてい",
    tags: [ "inazuma", "facility" ],
    notes: "稲妻城にある料理屋",
  },
  {
    en: "Tsukumomono Groceries",
    ja: "九十九物",
    zhCN: "九十九物",
    zhTW: "九十九物",
    pronunciationJa: "つくももの",
    tags: [ "inazuma", "facility" ],
    notes: "稲妻城にある店",
  },
  {
    en: "Yayoi Lacquerware",
    ja: "弥生漆器",
    zhCN: "弥生漆器",
    zhTW: "彌生漆器",
    pronunciationJa: "やよいしっき",
    tags: [ "inazuma", "facility" ],
    notes: "稲妻城にある店",
  },
  {
    en: "Ogura Textiles & Kimono",
    ja: "小倉屋",
    zhCN: "小仓屋",
    zhTW: "小倉屋",
    pronunciationJa: "おぐらや",
    tags: [ "inazuma", "facility" ],
    notes: "稲妻城にある店",
  },

  //
  // Sumeru
  //
  {
    en: "Zubayr Theater",
    ja: "ズバイルシアター",
    zhCN: "祖拜尔剧场",
    zhTW: "祖拜爾劇場",
    tags: [ "sumeru", "facility" ],
  },
  {
    en: "Puspa Café",
    ja: "プスパカフェ",
    zhCN: "普斯帕咖啡馆",
    zhTW: "普斯帕咖啡館",
    tags: [ "sumeru", "facility" ],
  },
  {
    en: "Citadel of Regzar",
    ja: "レグザー庁",
    zhCN: "聚砂厅",
    zhTW: "聚砂廳",
    tags: [ "sumeru", "facility" ],
  },
  {
    en: "Lambad's Tavern",
    ja: "ランバド酒場",
    zhCN: "兰巴德酒馆",
    zhTW: "蘭巴德酒館",
    tags: [ "sumeru", "facility" ],
  },
  {
    en: "Menakeri's Treasure Shop",
    ja: "メナケリーショップ",
    zhCN: "梅娜卡里商铺",
    zhTW: "梅娜卡里商鋪",
    tags: [ "sumeru", "facility" ],
    notes: "草の印をアイテムと交換できる店",
  },
  {
    en: "Bimarstan",
    ja: "ビマリスタン",
    zhCN: "健康之家",
    zhTW: "健康之家",
    tags: [ "sumeru", "facility" ],
  },
  {
    en: "House of Daena",
    ja: "知恵の殿堂",
    zhCN: "智慧宫",
    zhTW: "智慧宮",
    tags: [ "sumeru", "facility" ],
    notes: "スメール教令院の奥にある図書館",
  },
  {
    en: "Djafar Tavern",
    ja: "ジャファータバーン",
    zhCN: "迪亚法饭店",
    zhTW: "迪亞法飯店",
    tags: [ "sumeru", "facility" ],
  },
  {
    en: "Wikala Funduq",
    ja: "ウィカラ隊商宿",
    zhCN: "维卡拉商栈",
    zhTW: "維卡拉商棧",
    pronunciationJa: "ウィカラたいしょうやど", // Need Confirmation
    tags: [ "sumeru", "facility" ],
  },
  {
    en: "Acara Crafts",
    ja: "アジャンタ彫刻店",
    zhCN: "阿折罗造像",
    zhTW: "阿折羅造像",
    pronunciationJa: "アジャンタちょうこくてん",
    tags: [ "sumeru", "facility" ],
  },
  {
    en: "Pharos Lighthouse",
    ja: "ファロス灯台",
    zhCN: "法逻斯灯塔",
    zhTW: "法邏斯燈塔",
    pronunciationJa: "ファロスとうだい",
    tags: [ "sumeru", "facility" ],
  },
  {
    en: "Amrita Pool",
    ja: "甘露の池",
    zhCN: "甘露池",
    zhTW: "甘露池",
    pronunciationJa: "かんろのいけ",
    tags: [ "sumeru", "facility" ],
  },

  //
  // Fontaine
  //
  {
    en: "Station Main Hall",
    ja: "セントラルポートホール",
    zhCN: "总站大厅",
    zhTW: "總站大廳",
    tags: [ "fontaine", "facility" ],
    notes: "フォンテーヌ廷の巡水船乗り場の1F",
  },
  {
    en: "Callas Line",
    ja: "カーレス線",
    zhCN: "卡雷斯线",
    zhTW: "卡雷斯線",
    tags: [ "fontaine", "facility" ],
    notes: "フォンテーヌの巡水船の線の1つ",
  },
  {
    en: "Clementine Line",
    ja: "クレメンタイン線",
    zhCN: "克莱门汀线",
    zhTW: "克萊門汀線",
    tags: [ "fontaine", "facility" ],
    notes: "フォンテーヌの巡水船の線の1つ",
  },
  {
    en: "Navia Line",
    ja: "ナヴィア線",
    zhCN: "娜维娅线",
    zhTW: "娜維婭線",
    tags: [ "fontaine", "facility" ],
    notes: "フォンテーヌの巡水船の線の1つ",
  },
  {
    en: "Romaritime Station",
    ja: "ポート・ロマリタイム",
    zhCN: "海露站",
    zhTW: "海露站",
    tags: [ "fontaine", "facility" ],
    notes: "英訳から察するに、「ロマリタイムハーバー」は国外からの船を含め、様々な船舶が停泊する港としての意味を示す一方、「ポート・ロマリタイム」は巡水船の駅を示していると思われる。",
  },
  {
    en: "Hotel Debord",
    ja: "ホテル・ドゥボール",
    zhCN: "德波大饭店",
    zhTW: "德波大飯店",
    tags: [ "fontaine", "facility" ],
  },
  {
    en: "Café Lucerne",
    ja: "カフェ・ルツェルン",
    zhCN: "露泽咖啡厅",
    zhTW: "露澤咖啡廳",
    tags: [ "fontaine", "facility" ],
  },
  {
    en: "Chioriya Boutique",
    ja: "千織屋",
    zhCN: "千织屋",
    zhTW: "千織屋",
    tags: [ "fontaine", "facility" ],
  },
  {
    en: "Bertin's House of Curiosities",
    ja: "バーティンの珍奇館",
    zhCN: "白棠珍奇屋",
    zhTW: "白棠珍奇屋",
    pronunciationJa: "バーティンのちんきかん",
    tags: [ "fontaine", "facility" ],
    notes: "水の印交換ショップ",
  },
  {
    en: "Beaumont Workshop",
    ja: "ボーモント工房",
    zhCN: "博蒙特工坊",
    zhTW: "博蒙特工坊",
    pronunciationJa: "ボーモントこうぼう",
    tags: [ "fontaine", "facility" ],
    notes: "フォンテーヌの鍛冶屋",
  },
  {
    en: "Leschots Clockwork Workshop",
    ja: "レシュッツのクロックワーク工房",
    zhCN: "莱舒的发条工坊",
    zhTW: "萊舒的發條工坊",
    pronunciationJa: "レシュッツのクロックワークこうぼう",
    tags: [ "fontaine", "facility" ],
  },
  {
    en: "Lepaute Horlogerie",
    ja: "ルポート時計店",
    zhCN: "勒波特钟表店",
    zhTW: "勒波特鐘錶店",
    pronunciationJa: "ルポートとけいてん",
    tags: [ "fontaine", "facility" ],
  },
  {
    en: "Damoville Purveyor",
    ja: "ダーモヴィル雑貨店",
    zhCN: "达莫维百货",
    zhTW: "達莫維百貨",
    pronunciationJa: "ダーモヴィルざっかてん",
    tags: [ "fontaine", "facility" ],
    // related: [ "boucicaut" ]
  },
  {
    en: "Coupon Cafeteria",
    ja: "特別許可食堂",
    zhCN: "特许食堂",
    zhTW: "特許食堂",
    pronunciationJa: "とくべつきょかしょくどう",
    tags: [ "fontaine", "facility" ],
    notes: "メロピデ要塞の食堂",
  },
  {
    en: "Welfare Meal",
    ja: "サービス食",
    zhCN: "福利餐",
    zhTW: "福利餐",
    pronunciationJa: "サービスしょく",
    tags: [ "fontaine" ],
    notes: "特別許可食堂で1日1食、無料で提供されるランダムな料理とおみくじのセット",
  },
  {
    en: "Rag and Bone Shop",
    ja: "ラグ&ボーンショップ",
    zhCN: "破烂骨头商店",
    zhTW: "破爛骨頭商店",
    tags: [ "fontaine", "facility" ],
    notes: "メロピデ要塞の非公認ショップ",
  },
  {
    en: "Pankration Ring",
    ja: "鉄拳闘技場",
    zhCN: "拳力斗技场",
    zhTW: "拳力鬥技場",
    tags: [ "fontaine", "facility" ],
  },
  {
    en: "Pankration",
    ja: "鉄拳闘技",
    zhCN: "拳力斗技",
    zhTW: "拳力鬥技",
    tags: [ "fontaine" ],
  },

  //
  // Natlan
  //
  {
    en: "Weary Inn",
    ja: "草臥の家",
    zhCN: "倦意之屋",
    zhTW: "倦意之屋",
    pronunciationJa: "くさぶしのいえ",
    tags: [ "natlan", "facility" ],
  },
  {
    en: "Zakan's Street Bites",
    ja: "ザカンの屋台",
    zhCN: "扎坎小吃摊",
    zhTW: "扎坎小吃攤",
    pronunciationJa: "ザカンのやたい",
    tags: [ "natlan", "facility" ],
  },
  {
    en: "Muhuru's Gemporium",
    ja: "ムフル宝石店",
    zhCN: "穆胡鲁宝石铺",
    zhTW: "穆胡魯寶石鋪",
    pronunciationJa: "ムフルほうせきてん",
    tags: [ "natlan", "facility" ],
  },
  {
    en: "Throne of the Primal Fire",
    ja: "原火の玉座",
    zhCN: "源火之圣座",
    zhTW: "源火之聖座",
    pronunciationJa: "げんかのぎょくざ",
    tags: [ "natlan", "facility" ],
  },
  {
    en: "Djembe Studios",
    ja: "ジャンペ楽器店",
    zhCN: "金贝乐棚",
    zhTW: "金貝樂棚",
    pronunciationJa: "ジャンペがっきてん",
    tags: [ "natlan", "facility" ],
  },
  {
    en: "\"Yumekizuki's Cozy Cornder\"",
    ja: "「夢見月のホッと一息」",
    zhCN: "「梦见月的舒心小站」",
    zhTW: "「夢見月的舒心小站」",
    pronunciationJa: "ゆめみづきのホッとひといき",
    tags: [ "natlan", "facility" ],
  },
  {
    en: "Shoal's Haven",
    ja: "魚たちの家",
    zhCN: "鱼群之家",
    zhTW: "魚群之家",
    pronunciationJa: "さかなたちのいえ",
    tags: [ "natlan", "facility" ],
  },
  {
    en: "Me & Dew",
    ja: "ドランクハニー",
    zhCN: "醅蜜",
    zhTW: "醅蜜",
    tags: [ "natlan", "facility" ],
  },
  {
    en: "Asha's Treasure Trove",
    ja: "アシャの宝庫",
    zhCN: "阿夏的宝库",
    zhTW: "阿夏的寶庫",
    pronunciationJa: "アシャのほうこ",
    tags: [ "natlan", "facility" ],
  },
  {
    en: "Sun",
    ja: "太陽",
    zhCN: "太阳",
    zhTW: "太陽",
    pronunciationJa: "たいよう",
    tags: [ "natlan", "facility" ],
  },
  {
    en: "Floral Whispers",
    ja: "フローラルウィスパー",
    zhCN: "繁花之语",
    zhTW: "繁花之語",
    tags: [ "natlan", "facility" ],
  },
  {
    en: "Street Fighting Fontainian",
    ja: "ストリート ファイティング フォンテニアン",
    zhCN: "灰河街斗士",
    zhTW: "灰河街鬥士",
    notes: "中国語での店名を直訳すると「サーンドル河のストリートファイター」",
    tags: [ "natlan", "facility" ],
  },

  //
  // Nod-Krai
  //
  {
    en: "The Flagship",
    ja: "フラッグシップ",
    zhCN: "旗舰",
    zhTW: "旗艦",
    notes: "ナシャタウンのバー・ホテル",
    tags: [ "nodkrai", "facility" ],
  },
  {
    en: "Speranza",
    ja: "スペランザ",
    zhCN: "斯偑兰扎",
    zhTW: "斯偑蘭扎",
    notes: "ナシャタウンの食堂",
    tags: [ "nodkrai", "facility" ],
  },
  {
    en: "Mead-Boy",
    ja: "ミードボーイ",
    zhCN: "蜜酒小子",
    zhTW: "蜜酒小子",
    notes: "ナシャタウンの店",
    tags: [ "nodkrai", "facility" ],
  },
  {
    en: "Nuts \"N\" Nuts",
    ja: "ナッツ＆ナット",
    zhCN: "坚果与螺钉",
    zhTW: "堅果與螺釘",
    notes: "ナシャタウンの店",
    tags: [ "nodkrai", "facility" ],
  },
  {
    en: "Mimisbrunnr Books",
    ja: "ミミール書店",
    zhCN: "秘谜书屋",
    zhTW: "秘謎書屋",
    notes: "ナシャタウンの店",
    tags: [ "nodkrai", "facility" ],
  },
  {
    en: "Voynich Guild",
    ja: "ヴォイニッチ商会",
    zhCN: "伏尼契商会",
    zhTW: "伏尼契商會",
    pronunciationJa: "ヴォイニッチしょうかい",
    notes: "ナシャタウンの店",
    tags: [ "nodkrai", "facility" ],
  },
  {
    en: "Vesnici Proleca Greenhouse",
    ja: "スプリング・ヴィレッジ",
    zhCN: "春景花房",
    zhTW: "春景花房",
    notes: "ナシャタウンの店",
    tags: [ "nodkrai", "facility" ],
  },

  //
  // Snezhnaya
  //

  // TODO need check
  {
    en: "Northland Bank",
    ja: "北国銀行",
    zhCN: "北国银行",
    zhTW: "北國銀行",
    pronunciationJa: "きたくにぎんこう",
    tags: [ "snezhnaya", "liyue", "fontaine", "nodkrai", "facility" ],
  },
] as const satisfies SourceWord[];
